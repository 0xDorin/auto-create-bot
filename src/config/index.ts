...
interface BotConfig {
  ...
  // Trading
  initialBuyMode: "fixed" | "dynamic";
  initialBuyAmount: string; // in MON (used when mode=fixed)
  targetPoints: number; // target points (used when mode=dynamic)
...
function validateEnv(): void {
  ...
  if (networkMode === "mainnet") {
    required.push(
      "MAINNET_RPC_URL",
      "MAINNET_CHAIN_ID",
      "MAINNET_METADATA_API_BASE_URL"
    );
  } else {
    required.push(
      "TESTNET_RPC_URL",
      "TESTNET_CHAIN_ID",
      "TESTNET_METADATA_API_BASE_URL"
    );
  }
...
function parseConfig(): BotConfig {
  ...
  const config: BotConfig = {
    ...
    metadataLimitPerPage: parseInt(
      process.env.METADATA_LIMIT_PER_PAGE || "100"
    ),
    ...
    initialBuyMode: (process.env.INITIAL_BUY_MODE || "fixed") as
      | "fixed"
      | "dynamic",
    initialBuyAmount: process.env.INITIAL_BUY_AMOUNT || "0.1",
    targetPoints: parseFloat(process.env.TARGET_POINTS || "20.5"),
  ...
  if (!["fixed", "dynamic"].includes(config.initialBuyMode)) {
