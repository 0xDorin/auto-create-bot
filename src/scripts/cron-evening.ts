/**
 * Cron job for Railway - Evening Mode
 * - Runs at 21:00 KST (12:00 UTC)
 * - Creates 30 tokens over 4 hours
...
/**
 * Evening mode configuration (hardcoded)
 */
const MODE_CONFIG = {
  name: 'evening',
  tokensToCreate: 30,
...
async function main() {
  ...
  console.log('\n' + '='.repeat(80));
  console.log('🤖 CRON JOB STARTED - EVENING MODE');
  console.log('='.repeat(80));
  console.log(`Time: ${new Date().toLocaleString()}`);
  ...
  try {
    ...
    if (startBalance < requiredBalance) {
      await sendAlert('Insufficient balance for evening cron run', {
    ...
    console.log('✅ Evening cron job completed successfully!\n');
    process.exit(0);
  } catch (error) {
    await sendAlert('Evening cron job failed', {
      mode: MODE_CONFIG.name,
      error: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : undefined,
    });

    console.error('\n❌ Evening cron job failed:', error);
    process.exit(1);
  }
}
