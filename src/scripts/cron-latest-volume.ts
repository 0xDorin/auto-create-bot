 ...
 */
async function getLatestTradeTokens(recentTokens: string[]): Promise<
  Array<{
    tokenAddress: string;
    symbol: string;
    name: string;
...
> {
  ...
  console.log(
    `  ✅ Available: ${available.length} (excluded ${recentTokens.length} recent)`
  );
...
async function main() {
  ...
  try {
    ...
    const pointsPerTrade =
      volumeConfig.targetPoints / volumeConfig.tradesPerWallet;
    ...
    console.log(
      `  Current wallet: ${state.currentWalletIndex + 1}/${
        volumeConfig.numWallets
      }`
    );
    console.log(
      `  Current wallet trades: ${state.currentWalletTrades}/${volumeConfig.tradesPerWallet}`
    );
    console.log();
    ...
    const volumeWallets = Array.from(
      { length: volumeConfig.numWallets },
      (_, i) => deriveWallet(volumeConfig.mnemonic, i + 1)
    );
    ...
    while (totalTrades < targetTrades) {
      ...
      try {
        ...
        recentTokens = [token.tokenAddress, ...recentTokens].slice(
          0,
      ...
      if (result.status === "success") {
        ...
        if (walletCompleted) {
          console.log(
            `\n  ✅ Wallet ${walletIndex + 1} completed ${
              volumeConfig.tradesPerWallet
            } trades`
          );
