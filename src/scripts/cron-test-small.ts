/**
 * Cron job for Railway - Test Small (Testnet)
 * - Runs every hour at :00 (0, 30 minutes)
 * - Creates 10 tokens over 20 minutes
...
/**
 * Test Small mode configuration (hardcoded)
 */
const MODE_CONFIG = {
  name: 'test-small',
  tokensToCreate: 10,
...
async function main() {
  ...
  console.log('\n' + '='.repeat(80));
  console.log('🤖 CRON JOB STARTED - TEST SMALL (10 tokens, 20min)');
  console.log('='.repeat(80));
  console.log(`Time: ${new Date().toLocaleString()}`);
  ...
  try {
    ...
    if (startBalance < requiredBalance) {
      await sendAlert('Insufficient balance for test-small run', {
    ...
    console.log('✅ Test Small cron job completed successfully!\n');
    process.exit(0);
  } catch (error) {
    await sendAlert('Test Small cron job failed', {
      mode: MODE_CONFIG.name,
      error: error instanceof Error ? error.message : String(error),
      stack: error instanceof Error ? error.stack : undefined,
    });

    console.error('\n❌ Test Small cron job failed:', error);
    process.exit(1);
  }
}
